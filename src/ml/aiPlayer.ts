...
export class SmartPlayer implements Player {
    ...
    constructor(
        ...
        private purchaseSharesBrain: Network
    ) { }
    ...
    play(gameState: IGameState, gameInstance: IAcquireGameInstance): PlayerAction {
        ...
        const purchaseShares = availableActions.find(a => a.type === 'ChooseShares') as ChooseShares | undefined;
        if (purchaseShares && purchaseShares.shares.length) {
            ...
            if (share) {
                ...
                if (other) {
                    return new RandomPlayer(this.playerId).actionFromAvailable(other, gameState);
                }
            }
        }
    ...
    private getInputs(gameState: IGameState, gameInstance: IAcquireGameInstance): number[] {
        ...
        const otherPlayer = gameInstance.playerIds.find(p => p !== this.playerId)?.[0] ?? '';

        ALL_HOTELS.forEach(h => {
            const shareDiff = (gameState.sharesState[this.playerId]?.[h] ?? 0) - (gameState.sharesState[otherPlayer]?.[h] ?? 0);
...
export class RandomPlayer implements Player {
    ...
    play(gameState: IGameState): PlayerAction {
        ...
        const endGame = gameState.availableActionsState.find(a => a.type === 'ChooseEndGame');
        if (endGame) {
    ...
    public actionFromAvailable(available: AvailableAction, gameState: IGameState): PlayerAction {
        let action: PlayerAction | null = null;
        switch (available.type) {
            case 'ChooseTile':
                const board = new Board(gameState.boardState);
                let tile = getRandomNumber(available.available)
                const starter = available.available.find(t => board.isHotelStarter(t))
            ...
            case 'ChooseHotelChain':
                action = PlayerActionType.StartHotelChain(this.playerId, available.hotelChains[0]);
                break;
            case 'ChooseShares':
                const random = Math.abs(Math.round(Math.random() * available.shares.length - 1));
                const share = available.shares[random] ?? available.shares[0]
                action = PlayerActionType.PurchaseShares(this.playerId, share.hotelChain);
                break;
            case 'ChooseMergeDirection':
                action = PlayerActionType.Merge(this.playerId, available.options[0], available.options.slice(1, -1));
                break;
            case 'ChooseToSellOrphanedShare':
                action = PlayerActionType.SellOrphanedShare(this.playerId, available.hotelChain);
                break;
        }
