  ...
  private playSingleGame(): string {
    ...
    while (!gameState.winners) {
      const player = this.players.find(
        (p) => p.playerId === gameState.currentPlayerIdState
      );
      if (!player) {
        throw new Error(
          "no player found for id: " + gameState.currentPlayerIdState
        );
      }
      let action;
      try {
        action = player.play(gameState, gameInstance);
        playedActions.push(action);
      } catch (e) {
        console.log(
          "could not play",
          BoardStateFactory.createDiagram(gameState.boardState)
        );
