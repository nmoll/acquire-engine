...
export class MockDatabaseClient implements DatabaseClient {
  ...
  updateActions(gameId: string, actions: PlayerAction[]): void {
    ...
    if (!callback) {
      throw new Error(
        `Expected callback for onActionsChanged with gameId: ${gameId}`
      );
    }
  ...
  mockNotifyGameChanged(gameId: string, game: IAcquireGameInstance | null) {
    ...
    if (!callback) {
      throw new Error(
        `Expected callback for onGameChanged with gameId: ${gameId}`
      );
    }
  ...
  mockDeleteGameResponse(gameId: string) {
    ...
    if (!callback) {
      throw new Error(
        `Expected callback for deleteGame with gameId: ${gameId}`
      );
    }
